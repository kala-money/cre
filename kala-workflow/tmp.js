...
var parsePPPResponse = (resp) => {
  const text = new TextDecoder().decode(resp.body);
  try {
    const json = JSON.parse(text);
    const pppVal = json.ppp;
    if (typeof pppVal === "number") {
      return BigInt(Math.round(pppVal * 1e8));
    }
    return 0n;
  } catch {
    return 0n;
...
var fetchOffchainData = (nodeRuntime) => {
  ...
  return indexValue * 10000000000n;
};
function updatePriceResult(runtime2, chainSelector, finalResult, evmConfig, t0, g0, s0, b0) {
  runtime2.log(`Updating price result to ${evmConfig.kalaConsumerAddress} with final result ${finalResult}`);
  const evmClient = new ClientCapability(chainSelector);
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 price, uint256 T0, uint256 G0, uint256 S0, uint256 B0"), [
    finalResult,
    t0,
    g0,
    s0,
  ...
  runtime2.log(`Updating price result with detail: ${finalResult}`);
  ...
  runtime2.log("Waiting for write response");
  const txHash = bytesToHex(writeReportResult.txHash || new Uint8Array(32));
  runtime2.log(`Write report transaction hash: ${txHash}`);
...
var onCronTrigger = (runtime2) => {
  ...
  const { t0, g0, s0, b0 } = getBaselines(runtime2.config);
  ...
  const offchainValue = runtime2.runInNodeMode(fetchOffchainData, consensusMedianAggregation())().result();
  runtime2.log(`Success fetch offchain value: ${offchainValue}`);
  const evmClient = new ClientCapability(network248.chainSelector.selector);
  const callData = encodeFunctionData({
    abi: KalaOracleAbi,
    functionName: "price"
  });
  const _onchainRead = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: evmConfig.kalaOracleAddress,
      data: callData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  });
  const finalResultValue = offchainValue;
  const txHash = updatePriceResult(runtime2, network248.chainSelector.selector, offchainValue, evmConfig, t0, g0, s0, b0);
  return {
    T0: t0,
    G0: g0,
    S0: s0,
